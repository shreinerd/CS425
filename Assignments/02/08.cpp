...
int main() {
    ...
    size_t maxIter[MaxThreads];  // Records the current maximum number of iterations
    Records records[MaxThreads]; // list of values that took maxIter iterations
    ...
    for (auto id = 0; id < MaxThreads; ++id) {
        std::thread t{[&,id]() {
            ...
            size_t  start;
            size_t count = std::max<size_t>(1, data.available() / 3500);

            while (data.getNext(count, start)) {
            ...
            maxIter[id] = myMaxIter;
            records[id] = myRecords;
    ...
    for (size_t id = 0; id < MaxThreads; ++id) {
        if (max.iter < maxIter[id]) {
            max.iter = maxIter[id];
            max.id = id;
        }
    }

    Records& record = records[max.id];
    // Output our final results
    std::cout << "\nmaximum number of iterations = " << max.iter << "\n";
    for (auto& [number, palindrome] : record) {
        std::cout 
            << "\t" << number
            << " : [" << palindrome.size() << "] "
            << palindrome << "\n";
